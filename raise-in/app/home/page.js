...
export default function Home() {
  ...
  useEffect(() => {
    const fetchFundings = async () => {
      try {
        const res = await fetch("/api/fundingDB")
        if (!res.ok) throw new Error("Failed to fetch fundings")
  ...
  const handleFundingAdded = async () => {
  try {
    const res = await fetch("/api/fundingDB")
    if (!res.ok) throw new Error("Failed to fetch fundings")
    const data = await res.json()
    ...
    console.error("Error fetching fundings in handleFundingAdded:", err)
  }
};
    ...
    <div className="w-full min-h-screen">
      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4">
        <div className="text-center max-w-2xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Create a Funding Campaign</h1>
          <p className="text-gray-400 mb-8 text-lg">Please login to start your funding campaign and help your cause reach more people.</p>
          <Link href="/login" className="inline-block px-8 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg hover:shadow-lg hover:shadow-blue-500/50 transition-all font-semibold">
            Login to Continue
          </Link>
          <p className="text-gray-500 mt-6">
            Don't have an account? <Link href="/login" className="text-blue-400 hover:text-blue-300">Sign up here</Link>
          </p>
        </div>
      </div>
    </div>
